...
public final class BinarySearchTreeMethods {
    ...
    /**
     * Returns the largest (right-most) label in t.
     *
     * @aliases reference returned by largest
     * @requires IS_BST(t) and |t| > 0
     * @ensures largest = [the largest label in t]
     */
    static <T> T largest(BinaryTree<T> t) {
        T largest = t.root();
        BinaryTree<T> left = t.newInstance();
        BinaryTree<T> right = t.newInstance();
        T root = t.disassemble(left, right);
     ...
     * @ensures largest = [the largest label in t]
     */
    static <T> T kthLargest(BinaryTree<T> t, int k) {
        T result = t.root();
        ...
        T root = t.disassemble(left, right);
        if (k > left.size()) {
            result = kthLargest(right, k - left.size() - 1);
        } else if (k < left.size()) {
            result = kthLargest(left, k);
        }
    ...
    public static void main(String[] args) {
        ...
        out.println("Largest:" + kthLargest(t, 0));
        out.println("t = " + BinaryTreeUtility.treeToString(t));
        out.println(t.equals(t1));
