        ...
        switch (this.kind()) {
            case BLOCK: {
                ...
                for (int i = 0; i < this.lengthOfBlock(); i++) {
                    Statement tmp = this.removeFromBlock(i);
                    tmp.prettyPrint(out, offset);
                    this.addToBlock(i, tmp);
                }
            ...
            case IF: {
                ...
                Statement tmp = this.newInstance();
                Condition c = this.disassembleIf(tmp);
                printSpaces(out, offset);
                out.println("IF " + toStringCondition(c) + " THEN");
                tmp.prettyPrint(out, offset + Program.INDENT_SIZE);
                printSpaces(out, offset);
                out.println("END IF");
                this.assembleIf(c, tmp);

                break;
            }
            case IF_ELSE: {

                Statement tmp1 = this.newInstance();
                Statement tmp2 = this.newInstance();
                Condition c = this.disassembleIfElse(tmp1, tmp2);
                printSpaces(out, offset);
                out.println("IF " + toStringCondition(c) + " THEN");
                tmp1.prettyPrint(out, offset + Program.INDENT_SIZE);
                printSpaces(out, offset);
                out.println("ELSE ");
                tmp2.prettyPrint(out, offset + Program.INDENT_SIZE);
                printSpaces(out, offset);
                out.println("END IF");
                this.assembleIfElse(c, tmp1, tmp2);
            ...
            case WHILE: {
                ...
                Statement tmp = this.newInstance();
                Condition c = this.disassembleWhile(tmp);
                printSpaces(out, offset);
                out.println("WHILE " + toStringCondition(c) + " DO");
                tmp.prettyPrint(out, offset + Program.INDENT_SIZE);
                printSpaces(out, offset);
                out.println("END WHILE");
                this.assembleWhile(c, tmp);
